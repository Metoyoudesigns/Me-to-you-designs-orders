...
function subscribeToOrders(){
    supabase.channel("dashboard-orders")
        ...
        .subscribe(status => console.log("Order realtime status:",status));
}
...
addJobButton?.addEventListener("click",()=>openEntryModal({
    type:"job", title:"Add Today's Job",
    subtitle:"Add a job you need to complete today.",
    placeholder:"e.g. Finish McKenzie's pyjamas"
}));

function addJob(v){ if(!v?.trim())return; jobs.push(v.trim()); saveJobs(); }
function saveJobs(){localStorage.setItem("todayJobs",JSON.stringify(jobs));loadJobs();}
function loadJobs(){
    if(!jobsList)return;
    jobsList.innerHTML="";
    jobs.forEach((job,index)=>{
        ...
        li.querySelector(".completeButton").onclick=()=>{jobs.splice(index,1);saveJobs();};
        li.querySelector(".deleteButton").onclick=()=>{if(confirm("Delete this job?")){jobs.splice(index,1);saveJobs();}};
...
addNoteButton?.addEventListener("click",()=>openEntryModal({
    type:"note", title:"Add Note",
    subtitle:"Add a note for your business dashboard.",
    placeholder:"Type your note here..."
}));

function addNote(v){if(!v?.trim())return;notes.push(v.trim());saveNotes();}
function saveNotes(){localStorage.setItem("notes",JSON.stringify(notes));loadNotes();}
function loadNotes(){
    if(!notesList)return;
    notesList.innerHTML="";
    notes.forEach((note,index)=>{
        ...
        li.querySelector(".editButton").onclick=()=>openEntryModal({
            type:"edit-note",title:"Edit Note",
            subtitle:"Update your note below.",
            placeholder:"Type your note here...",value:notes[index],
            onSave:updated=>{
                if(!updated.trim())notes.splice(index,1);
                else notes[index]=updated.trim();
                saveNotes();
            }
        });
        li.querySelector(".deleteButton").onclick=()=>{
            if(confirm("Delete this note?")){notes.splice(index,1);saveNotes();}
        };
...
function getOrderTotal(order){
    const value=order.order_total ?? order.orderTotal ?? order.total ?? order.total_amount ?? 0;
    const n=Number(String(value).replace(/[£,]/g,""));
    return Number.isFinite(n)?n:0;
}

function getDeposit(order){
    const value=order.deposit ?? order.deposit_amount ?? order.depositAmount ?? 0;
    const n=Number(String(value).replace(/[£,]/g,""));
    return Number.isFinite(n)?n:0;
}

function getBalance(order){
    const explicit=order.remaining_balance ?? order.balance ?? order.remainingBalance;
    if(explicit!==undefined && explicit!==null && explicit!==""){
        const n=Number(String(explicit).replace(/[£,]/g,""));
...
function isActive(order){
    const s=String(order.order_status ?? order.orderStatus ?? "").toLowerCase();
...
function updateStats(){
    ...
    if(pending)pending.textContent=orders.filter(isActive).length;
}

// FIXED: week means the CURRENT Monday-Sunday week.
// It does not mean "next 7 days".
...
function loadNextOrder(){
    const box=document.getElementById("nextOrderCard");
    if(!box)return;

    const active=orders
        .map(order=>({order,due:getDateNeeded(order)}))
        .filter(x=>x.due && isActive(x.order))
        .sort((a,b)=>a.due-b.due);

    if(!active.length){box.innerHTML="<p>No upcoming orders.</p>";return;}
    ...
    let dueText=days<0?`⚠️ ${Math.abs(days)} day(s) overdue`:
        days===0?"Today":`${days} day(s) remaining`;

    const orderNumber=order.order_number||"Order";
    const customerName=order.customer_name||"Customer";
    const status=order.order_status||"Unknown";

    box.innerHTML=`<h3></h3><p><strong></strong></p>
        <p>📅 ${due.toLocaleDateString("en-GB")}</p><p></p><p>✨ </p>`;
    box.querySelector("h3").textContent=orderNumber;
    box.querySelector("strong").textContent=customerName;
    box.querySelectorAll("p")[2].textContent=dueText;
    box.querySelectorAll("p")[3].textContent=`✨ ${status}`;
}
...
function loadOutstandingPayments(){
    ...
    if(!list)return;
    const outstanding=orders.map(order=>({order,balance:getBalance(order)}).filter?.(()=>true));
    const rows=orders.map(order=>({order,balance:getBalance(order)})).filter(x=>x.balance>0);

    if(!rows.length){list.innerHTML="<p>No outstanding payments.</p>";return;}
    list.innerHTML="";
    rows.sort((a,b)=>b.balance-a.balance).forEach(({order,balance})=>{
        const row=document.createElement("div");
        row.style.padding="10px 0";
        row.style.borderBottom="1px solid #f3e5ec";
        row.innerHTML="<strong></strong><span style='display:block;margin-top:4px;'></span>";
        row.querySelector("strong").textContent=`${order.order_number||"Order"} — ${order.customer_name||"Customer"}`;
        row.querySelector("span").textContent=`£${balance.toFixed(2)} outstanding`;
        list.appendChild(row);
    });
}

// REVENUE FIX: use order_date, which the Create Order page actually saves.
// Today / This Week / This Month are based on the date the order was created.
// Total includes every order.
function calculateRevenue(){
    ...
    orders.forEach(order=>{
        ...
        if(orderDate>=todayStart && orderDate<tomorrow)today+=amount;
        if(orderDate>=weekStart && orderDate<nextWeek)week+=amount;
        if(orderDate>=monthStart && orderDate<nextMonth)month+=amount;
    });
...
function openEntryModal({type,title,subtitle,placeholder,value="",onSave=null}){
    ...
    setTimeout(()=>{
        ...
        entryModalInput.setSelectionRange(entryModalInput.value.length,entryModalInput.value.length);
    },50);
}
...
saveEntryModal?.addEventListener("click",()=>{
    ...
    if(!value){entryModalInput.focus();return;}
...
entryModalInput?.addEventListener("keydown",e=>{
    if((e.ctrlKey||e.metaKey)&&e.key==="Enter")saveEntryModal.click();
});
document.addEventListener("keydown",e=>{if(e.key==="Escape")closeEntryModalWindow();});
