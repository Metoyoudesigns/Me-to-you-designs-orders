...
async function loadOrders(){
 ...
 orders=(data||[]).map(order=>({id:order.id,...order}));
 updateDashboard();
}
...
function loadJobs(){
 ...
 jobs.forEach((job,index)=>{
  ...
  li.innerHTML=`<span></span><div><button class="completeButton" title="Complete">✅</button><button class="deleteButton" title="Delete">🗑️</button></div>`;
  li.querySelector("span").textContent=job;
  li.querySelector(".completeButton").addEventListener("click",()=>{
   jobs.splice(index,1);saveJobs();
  });
...
function loadNotes(){
 ...
 notes.forEach((note,index)=>{
  const li=document.createElement("li");
  li.innerHTML=`<span></span><div><button class="editButton" title="Edit">✏️</button><button class="deleteButton" title="Delete">🗑️</button></div>`;
...
/*
 * IMPORTANT DATE FIX:
 * Prefer the actual order date saved by Create Order.
 * If an older order has no order_date, fall back to created_at/date.
 */
...
function getDeposit(order){
 const value=order.deposit??order.depositAmount??order.deposit_amount??0;
 const number=Number(String(value).replace(/[£,]/g,""));
 return Number.isFinite(number)?number:0;
}
...
function calculateDueDates(){
 ...
 orders.forEach(order=>{
  ...
  if(dueDay>=today&&dueDay<=weekEnd)week++;
 ...
 const weekEl=document.getElementById("dueWeek");
...
function loadNextOrder(){
 ...
 let dueText="";
 if(days<0)dueText=`⚠️ ${Math.abs(days)} day(s) overdue`;
 else if(days===0)dueText="Today";
 else dueText=`${days} day(s) remaining`;

 const orderNumber=next.order_number??next.orderNumber??"Order";
 const customerName=next.customer_name??next.customerName??"Customer";
 const status=next.order_status??next.orderStatus??"Unknown";
...
function loadOutstandingPayments(){
 ...
 .map(order=>({order,balance:getBalance(order)}))
 ...
 outstanding.sort((a,b)=>b.balance-a.balance).forEach(({order,balance})=>{
  const row=document.createElement("div");
  row.style.padding="10px 0";
  row.style.borderBottom="1px solid #f3e5ec";

  const orderNumber=order.order_number??order.orderNumber??"Order";
  const customerName=order.customer_name??order.customerName??"Customer";

  row.innerHTML=`<strong></strong><span style="display:block;margin-top:4px;">£${balance.toFixed(2)} outstanding</span>`;
...
/*
 * FIXED REVENUE:
 * Revenue is calculated from the saved ORDER DATE, not the date-needed-by.
 * This means an order created today counts in Today, This Week and This Month.
 * Older orders still contribute to Total.
 */
function calculateRevenue(){
 ...
 orders.forEach(order=>{
  ...
  const day=startOfDay(orderDate);

  if(day>=todayStart&&day<tomorrow)today+=amount;
  if(day>=weekStart&&day<nextWeek)week+=amount;
  if(day>=monthStart&&day<nextMonth)month+=amount;
 });
 ...
 if(monthEl)monthEl.textContent=`£${month.toFixed(2)}`;
